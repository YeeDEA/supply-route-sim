...
*Work period: Sep 15 – Oct 2, 2024, in Google Colab. Published here Aug 2026, when those notebooks were reorganized into this repository — so the git history starts at that import, not at the work. File-by-file mapping in [Provenance](#provenance).*
...
Thin argparse CLIs in `scripts/` (`run_qlearning`, `run_ppo`, `solve_cvrp`, `build_route_map`, `draw_flowchart`) call the same logic from `src/`.
